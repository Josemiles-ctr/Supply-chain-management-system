            ...
            ->schema([
                ...
                TextInput::make('price_per_unit')
                    ...
                    ->afterStateHydrated(function ($component, $state, $set, $get) {
                        // Set price if raw material is already selected
                        $rawMaterialId = $get('rawMaterial_id');
                        if ($rawMaterialId) {
                            $price = RawMaterial::find($rawMaterialId)?->price ?? null;
                            $set('price_per_unit', $price);
                        }
                    })
                    ->afterStateUpdated(function ($state, $set, $get) {
                        // If raw material is removed, clear price
                        if (!$get('rawMaterial_id')) {
                    ...
                    ->formatStateUsing(function ($state, $get) {
                        $rawMaterialId = $get('rawMaterial_id');
                        if ($rawMaterialId) {
                            return RawMaterial::find($rawMaterialId)?->price ?? null;
                        }
                        return null;
                    }),
