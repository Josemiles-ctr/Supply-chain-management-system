...
class SupplierReport extends Command
{
    ...
    public function handle()
    {
        ...
        $totalSales = RawMaterialsPurchaseOrder::sum('total_price')->where('status', 'delivered')->orWhere('status', 'confirmed');
        $this->info('Executing the supplier report command...');
        Mail::to('josephotai209@gmail.com')->send(new SendSupplierReport($supplier, $pendingOrdersCount, $deliveredOrdersCount, $confirmedOrdersCount, $cancelledOrdersCount, $totalOrdersCount, $totalSales, $reportDate   ));
    }
}