...
class ManufacturerReport extends Command
{
    ...
    public function handle()
    {
        ...
        $low = RawMaterial::where(function ($query) {
            $query->where('current_stock', '>=', (int)'reorder_level' * 2)->where('current_stock', '<', ((int)'reorder_level'*2) );
        })->count();
        ...
        Mail::to('josephotai209@gmail.com')->send(new SendManufacturerReport( $user, $deliveredCount, $pendingCount, $confirmedCount, $cancelledCount, $totalCount,$still, $low,$out,da));
    }
}