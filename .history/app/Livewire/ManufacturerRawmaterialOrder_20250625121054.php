...
class ManufacturerRawmaterialOrder extends Component
{
    ...
    public function reOrder($orderId)
    {
        ...
        if ($order) {
            // Here you can implement the logic to reorder, e.g., create a new order with the same details
            $newOrder = $order->replicate();
            $newOrder->status = 'pending'; // Reset status for new order
            $newOrder->save();
            session()->flash('message', 'Reorder successful!');
        } else {
