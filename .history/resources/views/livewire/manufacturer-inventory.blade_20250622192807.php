...
>
  <!-- Dark Background (does NOT close the modal) -->
  <div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 bg-opacity-50 pointer-events-auto"></div>

  <!-- Modal Content (closes only on outside click) -->
  <div 
    class="relative z-50 bg-white dark:bg-gray-900 w-full max-w-lg rounded-xl shadow-lg p-6 pointer-events-auto"
  ...
  >
    ...
    <div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 bg-opacity-30 pointer-events-auto"></div>
    ...
    <div @click.away="open = false"
      ...
      <form @submit.prevent="alert('Form submitted!'); open = false;">
        ...
        <div class="mt-6 flex justify-end space-x-3">
          <button type="button"
                  @click="open = false"
                  class="px-4 py-2 bg-gray-200 text-gray-800 cursor-pointer rounded-lg hover:bg-gray-300 transition">
            Cancel
          </button>

          <button type="submit"
                  class="px-4 py-2 bg-blue-600 text-white cursor-pointer rounded-lg hover:bg-blue-700 transition">
            Finish
          </button>
        </div>
      </form>
  ...
  <div x-show="open==='openedit'"
    ...
    <!-- Dark Background -->
    <div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 bg-opacity-50 pointer-events-auto"
         @click="open = false"></div>

    <!-- Modal Box -->
    <div class="relative z-10 bg-white w-full max-w-lg rounded-xl shadow-lg p-6 pointer-events-auto"
         x-transition
         @keydown.escape.window="open = false"
         @click.stop> <!-- This stops click propagation -->
      
      <!-- Modal Header -->
      <h2 class="text-xl font-semibold text-gray-800 mb-4">Edit product</h2>

      <!-- Form Inside Modal -->
      <form @submit.prevent="submitForm()"> <!-- Changed to call a function -->
        @csrf
        <div class="space-y-4">
          <!-- Name Field -->
          <div class="max-w-full mx-auto">
            ...
            <input type="text" id="name" name="name" required
                   class="mt-1 block w-full px-4 py-2 border text-black border-gray-300 rounded-lg focus:ring focus:ring-blue-200"
                   x-model="formData.name"> <!-- Added x-model -->
          </div>
          ...
          <div>
            ...
            <input type="number" id="quantity" name="quantity" required min="1"
                   class="mt-1 block w-full px-4 py-2 border text-black border-gray-300 rounded-lg focus:ring focus:ring-blue-200"
                   x-model="formData.quantity"> <!-- Added x-model -->
          </div>
        </div>
        
        <label for="message" class="block text-sm font-medium text-gray-700 mb-1 mt-4">Message</label>
        <textarea 
          id="message" 
          name="message"
          rows="4" 
          class="resize-none w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring focus:ring-blue-200 text-sm text-gray-800"
          placeholder="Enter your message..."
          x-model="formData.message"></textarea> <!-- Added x-model -->
