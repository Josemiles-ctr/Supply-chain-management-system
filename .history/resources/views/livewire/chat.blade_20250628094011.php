<x-layouts.dashboard-component-heading class="bg-white dark:bg-gray-800"
    title="Chats And Messages"
    description="Manage your chats and messages"
>
    <div class="filament-page px-0 py-0 !bg-transparent">
        <div class="chat-component flex h-[550px] text-sm border rounded-xl shadow overflow-hidden bg-green-200 dark:bg-gray-900">
            <div class="w-1/4 border-r bg-green-200 dark:bg-gray-800 flex flex-col">
            ...
            <div class="p-4 space-y-2">
                @foreach($messages as $message)
                    <div class="flex {{ $message->sender_id === Auth::id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-xs px-4 py-2 rounded-sm shadow {{ $message->sender_id === Auth::id() ? 'bg-gray-500 text-white dark:bg-gray-700' : 'bg-blue-600 text-white dark:bg-blue-700' }}">
                            {{ $message->message }}
                            <span class="text-gray-400 dark:text-gray-300 text-xs block mt-1">
                                {{ $message->created_at->diffForHumans() }}
                            </span>
                            @if($message->sender_id === Auth::id())
                                <div class="text-green-500 dark:text-green-300 text-xs mt-1 rounded-sm flex items-center gap-1">
                                    (You)
