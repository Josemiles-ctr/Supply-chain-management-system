        ...
        @foreach($rawmaterials as $rawmaterial)
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            $styles= [
              ...
              {{$rawmaterial->current_stock > $rawmaterial->reoderlevel + 11}} => ' bg-green-500'
             ]
             $statements=[
              {{$rawmaterial->current_stock < $rawmaterial->reoderlevel}} => 'Out Of Stock',

              {{$rawmaterial->current_stock <= $rawmaterial->reoderlevel + 10 }} => ' bg-orange-500',
              {{$rawmaterial->current_stock > $rawmaterial->reoderlevel + 11}} => ' Running Out Of Stock'

             ]
              @endphp
              <td class="px-6 py-4">
                  <div class="flex items-center">
                      <div class="h-2.5 w-2.5 rounded-full {{$styles}} me-2"></div> Online
                  </div>
              </td>
              <td class="px-6 py-4">
                  <!-- Modal toggle -->
                  <a @click="open = 'openedit'" href="#" type="button" data-modal-target="editUserModal" data-modal-show="editUserModal" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit user</a>
              </td>
          </tr>
      </tbody>
  </table>
