        ...
        @else
            <div class="p-4 space-y-2">
                @foreach($messages as $message)
                    @php
                        // Custom rounded corners for sent and received messages
                        $isSent = $message->sender_id === Auth::id();
                        $bubbleClasses = $isSent
                            ? 'bg-gray-500 text-white dark:bg-gray-700 rounded-2xl rounded-br-md'
                            : 'bg-blue-600 text-white dark:bg-blue-700 rounded-2xl rounded-bl-md';
                    @endphp
                    <div class="flex {{ $isSent ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-xs px-4 py-2 shadow {{ $bubbleClasses }}">
                            {{ $message->message }}
