          ...
          <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
            ...
            <td class="px-6 py-4">
              ...
              <td>
                ...
                <form wire:submit.prevent="reorder({{ $order->id }})" class="inline">
                  <button type="submit"
                          class="px-3 py-1.5 bg-red-400 text-white rounded-lg hover:bg-red-700 transition">
                      Cancel
                  </button>
                </form>
                  @elseif($order->status == 'completed')
                  <div type="button"
                          class="px-3 py-1.5 bg-green-400 text-white rounded-lg hover:bg-green-700 transition">
                      Completed
                  </div>
                  @elseif($order->status == 'cancelled')
                  <div type="button"
                          class="px-3 py-1.5 text-white rounded-lg hover:bg-red-700 transition">
                      Cancelled
                  </div>
                  @elseif($order->status == 'confirmed')
                  <div type="button"
                          class="px-3 py-1.5 bg-blue-400 text-white rounded-lg hover:bg-blue-700 transition">
                      Confirmed
                  </div>
                  @endif
